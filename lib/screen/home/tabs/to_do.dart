...
class _ToDoScreenState extends State<ToDoScreen> with WidgetsBindingObserver {
    ...
    Future saveCurrentDateTime(bool isPlay) async{
      ...
      PrefTimeModel prefTimeModel1 = PrefTimeModel(isPlay: isPlay, startDateTime: DateTime.now().toString(),endDateTime: DateTime.now().toString());
      prefTimeModel = PrefTimeModel(isPlay: isPlay, startDateTime: DateTime.now().toString(),endDateTime: DateTime.now().toString());
  ...
  startTimer() async{
    saveCurrentDateTime(true);
     timer = Timer.periodic(const Duration(seconds: 1), (time) async {
        ...
        if(prefTimeModel!.isPlay){
          currentTotalVal = DateTime.now().difference(DateTime.parse(prefTimeModel!.startDateTime)).inSeconds;
          currentTotalVal ++;
        }else{
          currentTotalVal = DateTime.parse(prefTimeModel!.endDateTime!).difference(DateTime.parse(prefTimeModel!.startDateTime)).inSeconds;
                              ...
                              InkWell(
                                onTap: () async{
                                  ...
                                  if(prefTimeModel != null){
                                    if(prefTimeModel!.isPlay){
                                      SharedPreferences preferences = await SharedPreferences.getInstance();
                                      PrefTimeModel prefTimeModel1 = PrefTimeModel(isPlay: false , startDateTime: DateTime.now().toString());
...
class PrefTimeModel{
  ...
  PrefTimeModel({required this.isPlay,required this.startDateTime,this.endDateTime});
